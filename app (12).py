...
if st.session_state.submitted:
    st.success(f"✅ **{st.session_state.get('submitted_name', '')}** 보호자님, 식단 검토 신청이 완료됐습니다!")
    st.markdown("""
<div style="background:#f0f9f0; border:1px solid #74c69d; border-radius:12px; padding:1.5rem; margin:1rem 0;">
    <h3 style="color:#2d6a4f; margin:0 0 0.5rem 0;">📬 검토 안내</h3>
    <p style="margin:0; color:#444;">
        검토 결과는 <b>신청 순서대로</b> 진행되며,<br>
        <b>영업일 기준 3~5일 이내 이메일</b>로 보내드립니다.<br><br>
        📷 사진(반려견 전신·식단·영양 라벨)은<br>
        <b>카카오채널 채팅창</b>으로 보내주세요.
    </p>
</div>
""", unsafe_allow_html=True)
...
if submit:
    ...
    elif not selected and not dry_diet:
        st.error("식단 재료를 하나 이상 선택해주세요. (건사료·동결건조 급여 시에는 선택하지 않아도 됩니다.)")
    else:
        # ── 영양 계산 ─────────────────────────────────────────────────────
