...
# ═══════════════════════════════════════════════════════════════════════════
st.markdown("---")
with st.expander("🔒 관리자 전용", expanded=False):
    admin_pw = st.text_input("관리자 비밀번호", type="password", key="admin_pw")
    admin_login = st.button("관리자 로그인", key="admin_login_btn")
    ...
    if st.session_state.get("admin_authenticated"):
        ...
        def update_sheet_cell(row_idx: int, col_name: str, value: str):
            """시트의 특정 셀 업데이트 (row_idx는 데이터 행 인덱스, 0-based)"""
            try:
                ws = get_gsheet()
                rows = ws.get_all_values()
                headers = rows[0]
                col_idx = headers.index(col_name) + 1  # 1-based
                ws.update_cell(row_idx + 2, col_idx, value)  # +2: 헤더+0-base
                return True
            except Exception as e:
                st.error(f"저장 실패: {e}")
                return False
        ...
        def recalculate(row_data: dict) -> tuple:
            """원자료에서 영양소 재계산 → (total_kcal, total_stats, mass_breakdown, der)"""
            weight = float(row_data.get("체중(kg)", 3))
            activity_val = float(row_data.get("활동계수", 1.6))
            rer = 70 * (weight ** 0.75)
            ...
            for _, frow in food_df.iterrows():
                ...
                for nutri in aafco_standards:
                    col_name = nutri.split("(")[0]
                    if col_name in frow:
                        total_stats[nutri] += frow[col_name] * ratio
        ...
        if not data_rows:
            ...
            # 신청 선택
            name_options = [
                f"{rd.get('신청일시','')} | {rd.get('이름','')} ({rd.get('검토상태','미검토')})"
                for rd in [dict(zip(headers, r)) for r in data_rows]
            ]
            ...
            with st.expander("📋 기본 정보", expanded=True):
                ...
                with c2:
                    ...
                    st.markdown(f"**활동계수:** {rd.get('활동계수','')}")
            ...
            with st.expander("📊 영양 분석 결과", expanded=True):
                ...
                if total_kcal_r > 0:
                    ...
                    for nutri, std in aafco_standards.items():
                        ...
                        if val < min_v:
                            status = "❌ 부족"
                        elif max_v and val > max_v:
                            status = "⚠️ 과잉"
                        else:
                            status = "✅ 적합"
                        aafco_rows.append({
                            "영양소": nutri,
                            "현재(1000kcal당)": f"{val:.2f}",
                            "AAFCO 최소": str(min_v),
                            "판정": status
                        })
