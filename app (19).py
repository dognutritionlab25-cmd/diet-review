...
">
    ...
    <div style="min-width:0;">
        ...
        <p style="margin:0; font-size:clamp(0.8rem, 2.5vw, 1rem); color:#666; font-weight:500;">
            전문가가 직접 분석해 드립니다
        </p>
    </div>
</div>
...
<div style="padding:1rem 0; border-bottom:2px solid #e8e8e8; margin-bottom:1.2rem;">
    <h1 style="margin:0; color:#3a2a1a; font-size:clamp(1.1rem, 4vw, 1.8rem); font-weight:900;">🐾 반려견 영양연구소 식단 검토 신청서</h1>
    <p style="color:#666; margin:0.3rem 0 0 0; font-size:1rem;">전문가가 직접 분석해 드립니다</p>
</div>
""", unsafe_allow_html=True)
...
def append_to_sheet(ws, row_dict: dict):
    """헤더가 없거나 헤더가 데이터와 다르면 1행에 헤더 삽입 후 데이터 추가"""
    try:
        existing = ws.get_all_values()
        headers = list(row_dict.keys())
        if not existing:
            # 시트가 완전히 비어있으면 헤더 + 데이터
            ws.append_row(headers)
            ws.append_row(list(row_dict.values()))
        else:
            first_row = existing[0]
            if first_row != headers:
                # 1행이 헤더가 아니면 맨 위에 헤더 행 삽입
...
db_data = [
    ...
    # ── v5.4 신규 ──
    {"재료명":"돼지 안심 (Pork Tenderloin)",  "category":"meat","bone_pct":0,"칼로리":109,"단백질":20.9,"지방":2.1, "칼슘":5,  "인":247,"철":0.97, "아연":1.88,"구리":0.094,"망간":0.012,"비타민A":0,   "비타민D":8,  "비타민E":0.22,"나트륨":53},
    {"재료명":"양간 (Lamb Liver)",            "category":"organ","bone_pct":0,"칼로리":139,"단백질":20.7,"지방":5.1, "칼슘":7,  "인":369,"철":7.5,  "아연":4.6, "구리":7.14, "망간":0.18, "비타민A":6990,"비타민D":0,  "비타민E":0,   "나트륨":71},
    {"재료명":"꿩 (Pheasant)",                "category":"meat","bone_pct":0,"칼로리":133,"단백질":23.6,"지방":3.6, "칼슘":12, "인":214,"철":1.1,  "아연":1.0, "구리":0.07, "망간":0.01, "비타민A":177, "비타민D":0,  "비타민E":0,   "나트륨":40},
    {"재료명":"토끼고기 (Rabbit)",            "category":"meat","bone_pct":0,"칼로리":136,"단백질":20.4,"지방":5.6, "칼슘":13, "인":216,"철":1.6,  "아연":1.6, "구리":0.14, "망간":0.04, "비타민A":0,   "비타민D":0,  "비타민E":0,   "나트륨":41},
    {"재료명":"치아씨드 (Chia Seeds)",        "category":"veggie","bone_pct":0,"칼로리":486,"단백질":16.5,"지방":30.7,"칼슘":631,"인":860,"철":7.7,  "아연":4.6, "구리":0.924,"망간":2.72, "비타민A":54,  "비타민D":0,  "비타민E":0.5, "나트륨":16},
    {"재료명":"메추리알 (Quail Egg)",         "category":"meat","bone_pct":0,"칼로리":158,"단백질":13.1,"지방":11.1,"칼슘":64, "인":226,"철":3.65, "아연":1.47,"구리":0.11, "망간":0.10, "비타민A":543, "비타민D":132,"비타민E":1.08,"나트륨":141},
    {"재료명":"연어 (Salmon)",                "category":"meat","bone_pct":0,"칼로리":208,"단백질":20.4,"지방":13.4,"칼슘":9,  "인":240,"철":0.34, "아연":0.36,"구리":0.05, "망간":0.01, "비타민A":50,  "비타민D":447,"비타민E":3.55,"나트륨":59},
]
...
<div style="background:#fff3e0; border-left:4px solid #ef6c00;
    ...
    <b>⚠️ 필수 입력 안내</b><br>
    원활한 식단 분석을 위해 <b>생식 계산기 입력 또는 하루 식단(재료명과 용량)</b>을
...
<div style="background:#e8f5e9; border-left:4px solid #388e3c;
    ...
    · 가금류(닭·오리·칠면조·꿩 등)는 <b>닭가슴살</b>로 대체<br>
    · 반추류(소·양·염소·사슴 등)는 <b>소고기</b>로 대체<br>
...
if submit:
    ...
    elif not selected and not has_extra and not dry_diet:
