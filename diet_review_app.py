...
def get_review_eligible_phones() -> set:
    """결제내역 탭 — 상품유형에 '식단분석' 포함된 전화번호 (5분 캐시)
    F열=전화번호(index 5), D열=상품유형(index 3). 헤더명 자동감지 + F열 fallback"""
    try:
        client = get_gspread_client()
        ...
        return {
            _norm_phone(row[pc_phone])
            for row in rows[1:]
            if len(row) > max(pc_phone, pc_type)
            and row[pc_phone].strip()
            and "식단분석" in row[pc_type].strip()
        }
    except Exception:
        return set()

def check_auth(phone: str) -> tuple[bool, str]:
    """전화번호 인증. (통과여부, 인증경로) 반환"""
    p = _norm_phone(phone)
    if p and p in get_review_eligible_phones():
        return True, "결제정보"
    return False, ""

def mark_review_used(phone: str):
...
with tab_admin:
    ...
    if st.session_state.get("admin_authenticated"):
        ...
        def parse_material_string(s: str) -> dict:
            """'닭가슴살:50g, 연어:30g' 형식 문자열을 {재료명: 원본값문자열} 딕셔너리로 파싱"""
            result = {}
            if not s:
                return result
        ...
        def recalculate(row_data: dict) -> tuple:
            ...
            materials = parse_material_string(row_data.get("선택재료", ""))
            for _, frow in food_df.iterrows():
                fname = frow["재료명"]
                if fname not in materials:
                    continue
                grams = _as_grams(materials[fname])
        ...
        if not data_rows:
            ...
            with st.expander("📋 기본 정보", expanded=True):
                ...
                consent_val = rd.get('콘텐츠활용동의', '미동의')
                consent_icon = "✅" if consent_val == "동의" else "➖"
                st.markdown(f"**콘텐츠 활용 동의:** {consent_icon} {consent_val}")
...
with tab_user:
    if not st.session_state.authenticated:
        ...
        if auth_btn:
            if not _norm_phone(auth_phone_input):
                ...
                ok, track = check_auth(auth_phone_input)
                if ok:
                    ...
                    st.error("❌ 이용권 구매 내역이 없는 전화번호입니다. 결제 시 입력한 전화번호를 다시 확인해주세요.")
        st.stop()
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    st.divider()
    st.subheader("📋 STEP 8. 콘텐츠 활용 동의 (선택)")
    content_consent = st.checkbox(
        "제출한 식단 정보, 사진 및 자료를 개인정보를 삭제하거나 익명 처리한 후 "
        "교육, 연구 및 콘텐츠 제작 사례로 활용하는 것에 동의합니다. (선택)"
    )
    st.caption("동의하지 않으셔도 식단 검토 서비스 이용에는 전혀 영향이 없습니다.")
        ...
        if not owner_name:
            ...
            if ws:
                row_dict = {
                    ...
                    "사진제출방식": "카카오채널",
                    "사진확인여부": "미확인",
                    "콘텐츠활용동의": "동의" if content_consent else "미동의",
                }
                ...
                if sheet_saved and st.session_state.get("auth_track") == "paid":
                    mark_review_used(st.session_state.get("auth_phone", ""))
